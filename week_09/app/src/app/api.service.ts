...
export class ApiService {
  ...
  constructor (private http: HttpClient, that) {
    // Store pointer which is passed in from the component. (parent caller)
    this._that = that;
  }
  ...
  getData (url, callback) {
    ...
    this.http.get<any>(url, { headers })
      ...
      .subscribe(
        result => { result.errorMessage = ""; callback(result, this._that) },
        error => callback({ errorMessage: JSON.stringify(error) }, this._that)
      )
  }
  ...
  postData (route, obj, callback) {
    ...
    this.http.post(route, obj, { headers })
      .subscribe(
